...
class RedHatSetupReturnType(enum.Enum):
    SUCCESS = "success"
    NAME_USED = "name_used"
...
class IntegrationsCore:
    ...
    ) -> RedHatSetupReturnType:
        if not await self.sources_client.is_source_name_valid(integrations_setup_name):
            return RedHatSetupReturnType.NAME_USED

        if self.sources_client.bulk_create(
            integrations_setup_name, redhat_cluster_identifier
        ):
            return RedHatSetupReturnType.SUCCESS

        return RedHatSetupReturnType.SETUP_ERROR
    ...
    async def communications_integrations_setup(
        ...
        endpoint_created = await self.integrations_client.create_endpoint(
